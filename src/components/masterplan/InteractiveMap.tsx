...
export default function InteractiveMap() {
  const [mapConfig, setMapConfig] = useState<any>({
    ...
    macroImage: '/areo.jpg', // 👈 Actualizado a formato .jpg horizontal
  ...
  useEffect(() => {
    const fetchMapData = async () => {
      ...
      if (errorZonas || errorLotes) {
        console.error("Error al cargar BD:", errorZonas, errorLotes);
        return;
      }
    ...
    const initPannellum = () => {
      ...
      if (viewState === '360_GLOBAL') {
        hotSpots = mapConfig.zones.map((zone: any) => ({
          ...
          clickHandlerFunc: () => {
            setActiveZone(zone); changeView('2D_MACRO');
          }
        }));
            ...
            <div className="absolute inset-0 z-30">
              <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="absolute inset-0 w-full h-full pointer-events-none">
                {viewState === '2D_MACRO' && mapConfig.zones[0]?.subZones?.map((sub: any) => (
                  <polygon key={sub.id} points={sub.polygon} onClick={(e) => { e.stopPropagation(); setActiveSubZone(sub); changeView('2D_MICRO'); }} className={`pointer-events-auto cursor-pointer stroke-[#FAFAF9] stroke-[0.2] transition-all ${activeSubZone?.id === sub.id ? 'fill-white opacity-40' : 'fill-white opacity-10 hover:opacity-30'}`} />
                ))}
