          ...
          <AnimatePresence>
            {viewState === '2D_MICRO' && activeLot && (
              <motion.div initial={{ x: 50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ opacity: 0, x: 50 }} className="absolute right-0 md:right-6 top-[auto] bottom-0 md:top-1/2 md:bottom-[auto] md:-translate-y-1/2 w-full md:w-80 bg-black/95 backdrop-blur-md border-t md:border border-[#292524] p-6 shadow-2xl z-40">
                ...
                <ul className="space-y-3 border-t border-[#292524] pt-4 mb-6 max-h-40 overflow-y-auto custom-scrollbar pr-2">
                  <li className="flex justify-between font-[family-name:var(--font-josefin)] text-sm text-[#A8A29E] mb-2 border-b border-[#292524] pb-2"><span>Superficie:</span> <span className="text-[#FAFAF9]">{activeLot.size}</span></li>
                  {activeLot.features?.map((f:string, i:number) => <li key={i} className="flex items-start gap-2 font-[family-name:var(--font-josefin)] text-xs text-[#FAFAF9] leading-tight"><span className="text-[#C9A962] mt-0.5">✓</span>{f}</li>)}
                </ul>
